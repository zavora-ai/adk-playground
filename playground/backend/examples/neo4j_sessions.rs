...
use adk_rust::session::{
    Neo4jSessionService, SessionService, CreateRequest, GetRequest, ListRequest, DeleteRequest,
};
...
async fn main() -> anyhow::Result<()> {
    ...
    let neo4j_pass = std::env::var("NEO4J_PASS")
        .unwrap_or_else(|_| "adk_playground".into());
    ...
    state1.insert("user:department".to_string(), serde_json::json!("engineering"));
    ...
    let eve_sessions = service.list(ListRequest {
        app_name: "neo4j-demo".into(),
        user_id: "eve".into(),
        limit: None,
        offset: None,
    }).await?;
    let frank_sessions = service.list(ListRequest {
        app_name: "neo4j-demo".into(),
        user_id: "frank".into(),
        limit: None,
        offset: None,
    ...
    let mut stream = runner.run(UserId::new("eve")?, SessionId::new("neo4j-session-1")?, msg1).await?;
    ...
    let mut stream2 = runner.run(UserId::new("eve")?, SessionId::new("neo4j-session-1")?, msg2).await?;
    ...
    println!("Session node `{}` — {} event nodes linked", retrieved.id(), retrieved.events().len());
